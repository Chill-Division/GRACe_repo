...
### 3. Genetics Names (`tests/test_genetics_duplicates.php`)
*   **No near-duplicates:** "white widow", "  WHITE WIDOW ", "WhiteWidow" and a tab in the middle are all recognised as the existing "White Widow" (the old check was an exact match, so they went in as extra genetics and split the reports). Accented capitals are handled without `mbstring`, which the add-on image doesn't have.
*   **Duplicates point at the original:** the result carries the existing genetics' id and name, so a form can select it.
*   **Tidy and sane:** extra spaces are removed before saving, and blank or over-long names are refused.

### 4. Quick Add (`tests/test_quick_add.php`)
*   **Wiring:** Receive plants and Record dry weight load `js/quick_add.js`, and put "+ Add new genetics…" at the bottom of the genetics list once it has loaded. The pop-up posts to `handle_quick_add_genetics.php`, which saves through `addGenetics()` (so near-duplicates are caught) and answers in JSON.
*   **Safe:** relative URLs only (Home Assistant ingress), and everything shown in the pop-up is escaped.
*   **In the browser (manual):** a new name is added in alphabetical order and selected, a near-duplicate selects the existing genetics, Cancel restores the previous choice, and anything already typed on the page is kept.

### 5. Permission Logic (`tests/test_permissions.php`)
...
### 6. Harvest Safeguards (`tests/test_harvest_safeguards.php`)
...
### 8. Stock On Hand and Entry Messages (`tests/test_entry_messages.php`)
*   **Say what happened:** receiving plants confirms "Added 12 White Widow plants. You now have 14 growing." (it used to say "Genetics received successfully"), and flower entries confirm the weight, genetics, reason, company and what's on hand now.
*   **Stock while you type:** `get_stock_on_hand.php` returns growing and drying plants and grams of flower for every genetics (`stockByGenetics()`). Receive plants and Record dry weight show it under the genetics list; the flower hint also shows the balance after the entry and turns red if a Subtract would take more than is on hand.
*   **Refusals:** receiving plants of an unknown genetics or with no count adds nothing.

### 9. Confirm Before Saving (`tests/test_confirm_entries.php`)
*   **Wiring:** Receive plants and Record dry weight show a `confirmAction()` summary (count and genetics; or weight, reason, company and what's on hand afterwards) and only submit after Confirm. Destroying flower gets the red button.
...
### 11. Saving Exactly Once (`tests/test_double_submit.php`)
*   **All or nothing:** a database trigger makes the third plant of a batch fail; receiving plants must report the error and leave none of the batch behind (it used to keep the plants inserted before the failure). With nothing failing, the whole batch goes in with one time stamp.
*   **No double taps:** `js/growcart.js` ignores further submits of a form that's already on its way (checked before any page's own handler, so no second confirm pop-up), and the confirm steps send through `submitOnce()`. Buttons are disabled only after the browser has read the form, so named submit buttons still send their value, and the Back button makes a page's forms usable again.

### 12. Recent Entries (`tests/test_recent_entries.php`)
*   **Receive plants:** one line per entry (a batch shares a time stamp), newest first, whatever has happened to the plants since, so the same batch entered twice shows up as two matching lines.
*   **Harvest / Destroy / Send:** one line per batch with its status and company, leaving out growing plants.
*   **Record dry weight:** every flower entry with its signed weight, reason, company, and the manifest number for manifest deductions.
*   **Shown safely:** dates appear as stored (NZ time) as day/month/year hours:minutes, only the requested number of lines are listed, and the pages escape what they show.

### 13. What's New (`tests/test_whats_new.php`)
*   **Reading the changelog:** the newest `## [x.y.z] - date` section of `grace_addon/CHANGELOG.md` is found (it must match `config.yaml`), stops where the previous version starts, and renders to HTML with headings, lists, bold and code, escaping everything else.
*   **New installs never see it:** the first run on an empty database records the current version as seen, and setting up the company afterwards doesn't change that.
*   **Upgrades see it once:** an install with company details or ledger entries from before What's new existed sees the latest notes. Once they're marked seen, nobody sees them again, on any device (it's stored in the `Settings` table as `whatsNewSeenVersion`). Only the current version can be marked seen.
*   **Only the latest:** after skipped versions only the newest notes show, going back to an older version shows nothing, and a missing changelog shows nothing without an error.
*   **Wiring:** the footer adds the pop-up (and its script) only when there's something to show, closing it posts to `handle_whats_new.php`, Administration → What's new shows the notes on demand, the Dockerfile copies `CHANGELOG.md` into the image, and `nav.php` doesn't leak its loop variables into pages.

### 14. Annual Stocktake Logic (`tests/test_annual_stocktake.php`)
...
### 15. Current Plants (`tests/test_current_plants.php`)
*   **Drying counts:** the Current Plants report counts growing and drying plants, because drying plants are still on site until they're sent or destroyed. It used to count growing plants only, so a crop that had been cut and was drying disappeared from it.
*   **Agrees with the stocktake:** each genetics' count matches the annual stocktake's end balance, and every plant status the database allows is either on hand or departed, never both.
*   **Every genetics listed:** alphabetically, with 0 where there are none, so "Hide rows with all zero values" still works. The page gets its numbers from `currentPlantCounts()` and says drying plants are included.

### 16. NZ Time (`tests/test_nz_time.php`)
...
### 17. Agency Report Reminders (`tests/test_report_reminders.php`)
...
### 18. Monthly Report Periods (`tests/test_report_periods.php`)
...
### 19. Company Adding (`tests/test_company_adding.php`)
*   **One way in:** Administration → Add Verified Company and the "+ Add new company…" pop-up (Harvest / Destroy / Send, Record dry weight, Generate Shipping Manifest) both save through `addVerifiedCompany()`.
*   **Already there:** a license number that's already on file is recognised as that company, and its details come back so the page can select it. Another company's contact email is refused, as when editing.
*   **Complete details:** every field is required, the email must be valid, and nothing is saved when something's missing.
*   **Still never deleted:** `company_lib.php` contains no delete. The manifest form makes you choose each company rather than defaulting to the first in the list.

### 20. Company Editing (`tests/test_company_editing.php`)
...
### 21. Own Company Details (`tests/test_own_company.php`)
...
### 22. License Alerts (`tests/test_license_alerts.php`)
...
### 23. License Expiry Limit (`tests/test_license_expiry.php`)
...
### 24. Download Filenames (`tests/test_download_names.php`)
...
### 25. Static Code Analysis (`tests/static_checks.sh`)
*   **Critical Paths:**
...
### 26. Version Consistency (`tests/test_version_consistency.php`)
*   **Why:** Ensures the version number is identical across:
    *   `config.yaml` (Home Assistant)
    *   `nav.php` (UI Display)
    *   `CHANGELOG.md` (Release Notes)

### 27. PHP Syntax Check (`tests/syntax_check.sh`)
...
*   [ ] **Confirm steps** (Receive plants, Record dry weight): pressing the button shows a summary; Cancel saves nothing and keeps the form filled in; Confirm saves and shows what was recorded.
*   [ ] **Large entries**: set Administration → Entry warning limits low (say 3 plants and 10 g), then receive 5 plants, record 20 g and generate a 12-plant manifest. Each confirm shows the red warning and Confirm stays disabled until the tick box is ticked. Put the limits back afterwards.
*   [ ] **What's new**: seed with `--legacy` (an upgraded install) and open any page. The pop-up shows the newest changelog section folded, "Show more…" opens it into a scrolling area, and after "Got it" it doesn't come back in any browser. A normal seed (a new install) never shows it. Administration → What's new shows the notes any time. Check it at phone size too.
